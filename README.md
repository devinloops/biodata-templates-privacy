Privacy Policy – Biodata Templates

Biodata Templates is a simple application that provides biodata templates for personal use.

Information Collection and Use

This app does not collect, store, or share any personal information from users.

Data Storage

All data entered into the app (if any) remains on the user’s device and is not transmitted to any server.

Third-Party Services

Biodata Templates does not use third-party services that collect user data.

Internet Connection

The app works offline and does not require an internet connection.

Children’s Privacy

This app is not specifically directed at children under the age of 13. We do not knowingly collect any personal information from children.

Changes to This Privacy Policy

This privacy policy may be updated from time to time. Any changes will be posted on this page.

Contact

If you have any questions about this Privacy Policy, you can contact us at:
devinloops@gmail.com
